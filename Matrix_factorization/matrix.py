...
def finite_consensus(r, matrix, agents, Gamma):
    ...
    d_min = min(d.values())
    for i in range(agents):
        phi[i].append(np.zeros(r[i].shape))
        for p, p1 in enumerate(degree_set[i]):
            phi[i][-1] += Gamma[i][0][p] * r[p1]
    for k in range(agents - d_min):
        r_new={i:np.zeros(r[i].shape) for i in range(agents)}
        for i in range(agents):
            for j in degree_set[i]:
                r_new[i]+=matrix[i][j]*r[j]
        r=copy.deepcopy(r_new)
        for i in range(agents):
            phi[i].append(phi[i][-1])
            for p, p1 in enumerate(degree_set[i]):
                if k<=agents-d[i]-1:
                    phi[i][-1] += Gamma[i][0][(k+1) * d[i] + p] * r[p1]
    for i in range(agents):
        Phi[i] = phi[i][agents - d[i]]
    return Phi
